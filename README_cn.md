...
客户端与服务端两端均强制约束数据包总长度（Header + Body），超限包直接丢弃、不关闭连接：

- **软上限**：默认 200KB，可配置（`tcp.max-packet-length` / `websocket.max-packet-length`）
- **硬上限**：500KB（硬编码），配置超过后强制钳制为 500KB
- 丢包打印 WARN 日志（长度/限制/包 hash）；可选 `log-oversize-content` 由独立守护线程懒启动打印超限包前 200 字节（默认关闭，全程无线程）

详见 MANUAL_cn.md「5.2 数据包大小限制」。
